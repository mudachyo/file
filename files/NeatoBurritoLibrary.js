...
NeatoLib.Changelog.compareVersions = function(name, changes) {
    ...
    updateData = NeatoLib.Data.load("kotanoffUpdateData", spacelessName, {}),
    unreadChanges = [],
...
NeatoLib.Changelog.createChangeWindow = function(name, changes, allChanges, newUpdateData) {
    ...
    document.getElementsByClassName("app")[0].insertAdjacentHTML("beforeend", `
        ...
        .kotanoff-update-item {
            padding: 10px;
        }

        .kotanoff-update-label {
            color: white;
            font-size: 35px;
        }
        
        .kotanoff-update-note {
            color: white;
            font-size: 25px;
            opacity: 0.75;
            line-height: 25px;
    ...
    document.getElementsByClassName("kotanoff-changelog-backdrop")[0].addEventListener("click", () => {
        if(newUpdateData != undefined) NeatoLib.Data.save("kotanoffUpdateData", spacelessName, newUpdateData);
    ...
    for(let i = 0; i < changes.length; i++){
        scroller.insertAdjacentHTML("afterbegin", `
            <div class="kotanoff-update-item">
                <p class="kotanoff-update-label">` + changes[i] + `</p><p class="kotanoff-update-note">`
...
NeatoLib.Settings.Elements.pluginNameLabel = function(name) {
    return `
        ...
        <h style="color: white;font-size: 30px;font-weight: bold;">${name} by kotanoff</h>`;
}
...
NeatoLib.Settings.Elements.createRadioGroup = function(id, label, choices, selectedChoice, callback, description = "") {
    ...
    for(var i = 0; i < choices.length; i++) {
        ...
        choiceButton.setAttribute("class", "kotanoff-checkbox-item");
        ...
        choiceButton.addEventListener("click", e => {
            ...
            var checkboxes = $(e.currentTarget.parentElement).find(`.kotanoff-checkbox-item > label > div`);
...
NeatoLib.Updates.check = function(plugin) {
    ...
    let url = "https://rawgit.com/kotanoff/BetterDiscordPlugins/master/" + plugin.getName().split(" ").join("") + ".plugin.js";
...
NeatoLib.Debug.reloadLib = function() {
    ...
    setTimeout(() => {
        ...
        lib.setAttribute("src", "https://rawgit.com/kotanoff/BetterDiscordPlugins/master/Lib/NeatoBurritoLibrary.js");
